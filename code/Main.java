...
public class Main {
	...
	public static void printChoresByChild(Child child) {
		for (Chore chore : child.getChores()) {
			if (chore != null) {
				System.out.println("\t \t" + chore.getName() + " Reward Amount: " + chore.getRewardAmount()
						+ " Completed: " + chore.isComplete());
			}
		}
	...
	public static String choresByChildToSting(Child child) {
		...
		for (Chore chore : child.getChores()) {
			if (chore != null) {
				str += "" + chore.getName() + " Reward Amount: " + chore.getRewardAmount() + " Completed: "
						+ chore.isComplete() + "\t";
			}
		}
		return str;
	...
	public static void main(String[] args) throws SQLException, InterruptedException {
		...
		loadCSVData();
		//System.out.println("--" + getChildByName(children,"Madison").getChores().toString());
		...
		final String[] NAMES = { "Avery", "Bailey", "Carter", "Dylan", "Ethan", "Finley", "Grayson", "Harper",
				"Isabella", "Jaxon", "Kennedy", "Liam", "Mason", "Noah", "Olivia", "Parker", "Quinn", "Riley", "Sophia",
				"Taylor", "Victoria", "Wyatt", "Xander", "Yasmine", "Zoe" };
		...
		final String[] CHORE_NAMES = { "Cut the grass", "Take out the trash", "Do the dishes", "Sweep the floor",
				"Clean the bathroom", "Wash the car", "Walk the dog", "Fold laundry", "Vacuum the carpets",
				"Clean the windows" };

		for (String choreName : CHORE_NAMES) {
			int rewardAmount = random.nextInt((MAX_REWARD_AMOUNT - MIN_REWARD_AMOUNT) / REWARD_AMOUNT_STEP + 1)
					* REWARD_AMOUNT_STEP + MIN_REWARD_AMOUNT;
			chores.add(new Chore(UUID.randomUUID(), choreName, rewardAmount, false,
					random.nextInt(MAX_RECURRENCE_FREQUENCY) + 1));
		}
		...
		final String[] REWARD_NAMES = { "Video game time", "Ice cream", "Toy", "Stickers", "Movie night", "Pizza party",
				"Extra allowance", "Lego set", "Board game", "Book" };

		for (String choreName : REWARD_NAMES) {
			String rewardName = REWARD_NAMES[random.nextInt(REWARD_NAMES.length)];
			int coinAmount = random.nextInt((MAX_COIN_AMOUNT - MIN_COIN_AMOUNT) / COIN_AMOUNT_STEP + 1)
					* COIN_AMOUNT_STEP + MIN_COIN_AMOUNT;
	...
	public static void parentMode() {
		...
		while (choice == 1) {
			...
			case 1:
				...
				in.nextLine();
				Child newChild = new Child(UUID.randomUUID(), cName, 0, par, new ArrayList<Reward>(),
						new ArrayList<Chore>());
			...
			case 3:
				...
				// }
				// System.out.println("enter the name of the child. (or 'ALL' to view all
				// children)");
				// cName = in.nextLine();
				// if(cName.equals("ALL")){
				for (Child i : parChildren) {
			...
			// System.out.println("What do you want to do now? 1: Back to Main Menu | 2:
			// Exit");
			// choice = in.nextInt();
			// if(choice==1) {
			// continue;
	...
	public static void childMode(Child child) {
		...
		while (choice == 1) {
			...
			case 1:
				...
				System.out.println("You succesfully completed " + chore.getName() + "! You now have "
						+ child.getCoinsAvailable() + "coins.");
				System.out.println("What do you want to do now?  1: Back to Main Menu | 2: Exit");
				choice = in.nextInt();
			...
			case 2:
				...
				while (!validReward) {
					...
					if (child.getCoinsAvailable() < reward.getCoinAmount()) {
						System.out.println("You don't have enough coins to buy " + reward.getName()
								+ ". Please buy something worth less than " + child.getCoinsAvailable() + " coins.");
					} else {
